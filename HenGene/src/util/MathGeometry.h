...
namespace math
{
	...
	template<typename T> constexpr T angle(const glm::tvec3<T> & v1, const glm::tvec3<T> & v2)
	{
		T length = glm::length(v1) * glm::length(v2);
		return length > T{ 0 } ? acos(glm::dot(v1, v2) / length) : T{ 0 };
	}

	template<typename T> constexpr glm::tvec3<T> cartesian(const glm::tvec2<T> & v)
	{
		return cartesian(v.x, v.y);
	}
	template<typename T> constexpr glm::tvec3<T> cartesian(const glm::tvec3<T> & v)
	{
		return cartesian(v.x, v.y);
	}
