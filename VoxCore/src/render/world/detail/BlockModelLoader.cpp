...
render::world::BlockModelLoader::BlockModelLoader(const glm::vec3 & offset, const glm::vec3 & rotation, const glm::vec3 & scale)
	: m_offset(offset), m_rotation(rotation), m_scale(scale)
{}

void render::world::BlockModelLoader::loadModel(const io::File & file)
{
...
void render::world::BlockModelLoader::loadGeometry(const pugi::xml_node & node)
{
	for (auto side = node.first_child(); side; side = side.next_sibling())
		loadSide(side, ::world::util::fromName(side.name()));
}
void render::world::BlockModelLoader::loadSide(const pugi::xml_node & node, const::world::util::Side & side)
{
	loadVertices(node.child(NODE_VERTICES), side);
	loadIndices(node.child(NODE_INDICES), side);
}
void render::world::BlockModelLoader::loadVertices(const pugi::xml_node & node, const::world::util::Side & side)
{
...
void render::world::BlockModelLoader::loadIndices(const pugi::xml_node & node, const::world::util::Side & side)
{
